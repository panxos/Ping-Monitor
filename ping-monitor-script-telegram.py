...
def main(host, log_file, interval, threshold_yellow, threshold_red, do_traceroute, telegram_token, telegram_chat_id):
    ...
    def signal_handler(sig, frame):
        ...
        if telegram_token and telegram_chat_id:
            notify_telegram(telegram_token, telegram_chat_id, "Monitoreo de red detenido.")
        sys.exit(0)
    ...
    if telegram_token and telegram_chat_id:
        notify_telegram(telegram_token, telegram_chat_id, f"Iniciando monitoreo de {host}")
        ...
        if latency is not None:
            ...
            if latency > threshold_red:
                ...
                if last_state != 'red':
                    ...
                    if telegram_token and telegram_chat_id:
                        notify_telegram(telegram_token, telegram_chat_id, f"ALERTA: Latencia crítica de {latency:.1f} ms")
                    last_state = 'red'
                ...
                if last_state != 'yellow':
                    ...
                    if telegram_token and telegram_chat_id:
                        notify_telegram(telegram_token, telegram_chat_id, f"Advertencia: Latencia media de {latency:.1f} ms")
                    last_state = 'yellow'
            else:
                ...
                if last_state != 'green':
                    ...
                    if telegram_token and telegram_chat_id:
                        notify_telegram(telegram_token, telegram_chat_id, f"Red normalizada: Latencia de {latency:.1f} ms")
                    last_state = 'green'
            
            if network_down:
                log_event("La red se ha recuperado", log_file)
                if telegram_token and telegram_chat_id:
                    notify_telegram(telegram_token, telegram_chat_id, "La red se ha recuperado")
                network_down = False
        else:
            ...
            if not network_down:
                ...
                if telegram_token and telegram_chat_id:
                    notify_telegram(telegram_token, telegram_chat_id, f"ALERTA: La red se ha caído. No se puede alcanzar {host}")
                network_down = True
        
        time.sleep(interval)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Monitor de red con sistema de semáforo y notificaciones Telegram")
    ...
    main(args.host, args.log, args.interval, args.yellow, args.red, args.tracer, args.telegram_token, args.telegram_chat_id)
